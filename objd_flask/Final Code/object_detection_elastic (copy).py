...
import es_pyspark_reading

#use GTR that is stored in Development
GTR = 'st_tw_7667'
CTR = '1'
target_type = 'twitter'
...
def initialize_config(spark):
    ...
    node_ip_ftp = '10.10.101.104'
    node_port_ftp = '21'
    auth_user_ftp = 'microcrawler'
    auth_pass_ftp = 'rapidev'

    node_ip_objd = '10.10.103.120'
    node_port_objd = '5002'
...
#df = es_pyspark_reading.search_col_by_index_fields_and_type(spark, elastic_index, GTR, columns="GTR,CTR,posts.media_c,posts.id",array_cols="posts.media_c")
...
def get_entities(list_of_urls):
    ...
    if list_of_urls == [] or list_of_urls == None or list_of_urls == ['default']:      # if media_c is empty / no-url (Checks for all platforms)
        results["urls"].append("")
