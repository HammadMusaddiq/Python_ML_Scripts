...
df = spark.read.json("/home/hammad/Downloads/objd_flask/st_rd_6857.json")
...
def get_entities(list_of_urls):
    ...
    if list_of_urls == []:      # if media_c in empty
        results["urls"].append("")
        ...
        try:
            for image_url in [url for urls in list_of_urls for url in urls]:       # for all platforms excepy reddit
            ...
            for image_url in [urls for urls in list_of_urls]:    # for reddit platform
                if image_url != None and image_url != ['default']:
