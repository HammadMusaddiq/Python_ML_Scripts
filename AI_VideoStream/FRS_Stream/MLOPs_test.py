...
emotionIP = "10.100.103.200"
IPADDRESS = "10.100.103.200"
ner_api_url = "10.100.103.200:5001"
text_c_ip = "10.100.103.200"
...
def emotion_microservices(x):
    ...
    start = time.time()
    response = requests.post(f"http://{emotionIP}:5016/", json={"text": str(x)})
    end = time.time()

    # print("==========================Emotion Request Ended ===========================\n\n")
    if response.status_code in [200,201]:
        print("Time Taken By Emosent: ", end-start)
    else:
        print("ERROR in Emosent!")
    return end-start


def object_detection(l):
    start = time.time()
    response = requests.post(f"http://{emotionIP}:5002/", json={"imageUrl": l})
    end = time.time()
    if response.status_code in [200,201]:
        print("Time Taken By Object Detection: ", end - start)
    return end - start


def weapon_detection(l):
    start = time.time()
    response = requests.post(f"http://{emotionIP}:5017/insert", json={"imageUrl": l})
    end = time.time()
    if response.status_code in [200,201]:
        print("Time Taken By Weapon Detection: ", end - start)
    else:
        print("ERROR in weapon Detection!")
    return end - start


def frs(l):
    start = time.time()
    response = requests.post(f"http://{emotionIP}:5003/", json={"imageUrl": l})
    end = time.time()
    if response.status_code in [200,201]:
        print("Time Taken By FRS: ", end - start)
    return end - start


if __name__ == '__main__':
    posts = ["Amber Heard, in her sob-story-defeat speech said I'm even more disappointed in what this verdict  means for \
