...
def plotAnnotation(recognized_boxes, missed_boxes, image_label, image):
    ...
    for mbox in missed_boxes:
        annotated_image = cv2.rectangle(annotated_image, (mbox[0], mbox[1]), (mbox[2]+mbox[0], mbox[3]+mbox[1]), (255,250,0), 3)   #mtcnn/retinanet
...
def readingFrames(selected_frames, frame, frames_list, counter, iteration_count, save_image, save_annotated_image, video_chunk):
    ...
    if counter in selected_frames:
        ...
        if matched_results['detected_person'] != [] or recognized_boxes != [] or missed_boxes != []:
...
def liveFeedOperations(live_feed, camera_fps, video_chunk, save_image, save_annotated_image, live_screen):
    ...
    while vid.isOpened():   
        ...
        matched_results, processed_image = readingFrames(selected_frames, frame, frames_list, counter, iteration_count, save_image, save_annotated_image, video_chunk)
        
        # generate alerts
        sendAlert(matched_results)
        
        # create video
        if video_chunk != 'False' and len(frames_list) == int(video_chunk):
            import pdb; pdb.set_trace()
