...
class Milvus_Stream:
    def __init__(self):
        ...
        self.collection_name = "ai_videostream_selected_3"
    ...
    def dumpToElasticSearch(self,idx,image_link):
        headers = {"Content-Type":"application/json"}
        current_time = datetime.datetime.now()
        c_time = current_time.strftime("%d/%m/%Y %H:%M:%S")
        data = {"date": c_time, "image_id":idx, "urls":image_link}
    ...
    def delete_data(self, id):
        """
            Delete Records from Milvus DB
        """
        logger.info("Milvus Database Embeddings Deletion Started.")
        try:
            collection = self.create_collection()
            logger.info("Milvus Database Connection is Successful.")
            ...
            try:
                ...
                if res:                    
                    result_deletion = collection.delete(exp)
                    logger.info("Embeddings have been deleted in the milvus database.")
                    return True
                else:
                    logger.info("No Embeddings present in the DB.")
                    return False
                ...
                error = "An Exception Occured while deleting data from the Milvus Database: {}".format(E)
                logger.error(error)
    ...
    def fetch_image(self,id):
        ...
        if success:
            try:
                images = response['hits']['hits'][0]['_source']['urls']
                date = response['hits']['hits'][0]['_source']['date']
                logger.info("Image URL has been fetched from ES.")
                return True, {"date":date,"urls":images,"success":success, "exception":exception}
