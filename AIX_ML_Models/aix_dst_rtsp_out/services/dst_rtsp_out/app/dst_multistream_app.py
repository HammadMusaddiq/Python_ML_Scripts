...
def calculate_iou(box1, box2):
    # Write code for calculating IoU
    x1, y1, w1, h1 = box1
    x2, y2, w2, h2 = box2
...
def get_sink(mode='display'):
    if mode == 'display':
        ...
        ## for CPU Encoder
        return 'nvvideoconvert ! capsfilter caps="video/x-raw, format=I420" ! x264enc ! queue ! rtph264pay ! udpsink host=224.224.255.255 port={} async=false sync=1 qos=0'.format(SPORT)
    else:
...
def get_tracker():
    return 'nvtracker tracker-width=640 tracker-height=384 gpu-id=0 \
...
def create_launch_string(camera_link):
    ...
    tracker_elem = get_tracker()

    infer_pipeline = f'nvstreammux name=mux batch-size=1 width=1920 height=1080 \
            ! nvinfer name=pgie unique-id=1 config-file-path=/app/configs/config_yolov5_pers.txt  \
            ! nvinfer name=pgie2 unique-id=2 config-file-path=/app/configs/config_yolov5_drone.txt  \
            ! {tracker_elem} \
...
def pgie_src_pad_buffer_probe(pad, info, u_data):
    global CURRENT_CAMERA_URL, producer, perf_data
    global CAMERA_THREATS, NAME, LAT, LONG, IMG_THREATS
        ...
        while l_obj is not None:
            ...
            try:
                ...
                detections.append({
                    ...
                    'event_id' : str(uuid.uuid4()) + str(obj_meta.object_id),
            ...
            if (frame_meta.frame_num % 30 == 0) and len(detections) > 0:
                ...
                # print(obj_meta.obj_label, obj_meta.object_id, obj_meta.unique_component_id)
                ...
                for idx, det in enumerate(detections):
                    for dic_threats in IMG_THREATS: #list of dict
                        for threat, level in dic_threats.items():
                            if det["label"].upper() == threat:
                                # print(f"Image saving...{det['label']}.\n")
                                
                                dx, dy, dw, dh = int(det["x"]), int(det["y"]), int(det["w"]), int(det["h"])
                                obj_id = det["object_id"]
                                
                                pre_dx, pre_dy, pre_dw, pre_dh = TRACKING_DICT.get(obj_id, (0, 0, 0, 0))

                                iou_val = calculate_iou((dx, dy, dw, dh), (pre_dx, pre_dy, pre_dw, pre_dh))

                                if iou_val < 0.1:
                                    TRACKING_DICT[obj_id] = (dx, dy, dw, dh)
                                    ...
                                    # sys.stdout.write("Image Added in Queue: " + str(det["label"]) + str(det["object_id"]) + '\n')
                                    # sys.stdout.flush()
                                    mutex.unlock()
...
if __name__ == '__main__':
    ...
    for threat, threat_id in threats_def.items():
        for th_id, th_level in CAMERA_THREATS.items():
            if threat_id == int(th_id): 
                IMG_THREATS.append({threat : th_level})
    ...
    sys.stdout.write("Input Link: " + str(cam_link) + '\n')
    sys.stdout.flush()  # flush the buffer to immediately send the output to the console

    cam_url = cam_link.encode().decode("utf-8")

    if "rtsp" in cam_url and "&subtype=0" not in cam_url:
        cam_url = cam_url + "&subtype=0"

    gst_launch_string = create_launch_string(camera_link=cam_url)

    sys.stdout.write("GST Launch String: " + str(gst_launch_string) + '\n')
    sys.stdout.flush()
