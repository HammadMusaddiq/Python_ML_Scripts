...
def image_toBytes(img_arr):
    ...
    # image_rgb = Image.fromarray(np.uint8(img_arr)).convert('RGB')
...
# def callingOBJD(image_url):
#     response = requests.post(f"http://{node_ip_objd}:{node_port_objd}/", json={"imageUrl": image_url}) # Raw data in Json
#     return response.json()["data"]

def callingWOD(image):
    
    # image = cv2.resize(image, (640, 640), cv2.INTER_AREA)
    # api_response = requests.post(f"http://{node_ip_wod}:{node_port_wod}/insert", files = {'image_path': image_toBytes(image)}) # Form data
...
def mergeObjects(image_shape, objd_data, wod_data):
    wod_boxes_act = []
    wod_score_act = []
    ...
    obj_dict = {}
    # obj_classified = objd_data["object_classified"] + wod_data["object_classified"]
    # obj_pred_axis = objd_data["predicted_axis"] + wod_boxes_act
    # obj_pro_score = objd_data["probability_score"] + wod_score_act
    # obj_dict.update({"object_classified": obj_classified, "predicted_axis": obj_pred_axis, "probability_score": obj_pro_score})
    ...
    if not isinstance(list_of_urls, list) or list_of_urls == [] or list_of_urls == None or list_of_urls == [
        ...
        for image_url in list_of_urls:  # for all platforms including reddit
            if image_url != [] and image_url != None and image_url != 'default':
                try:
                    # objd_res = callingOBJD(image_url)
                    # objd_data = objd_res[0]
                ...
                try:
                    object_data = mergeObjects(img_arr.shape, "objd_data", wod_data)
...
if __name__ == '__main__':
    ...
    node_ip_ftp = "10.100.102.114"
    node_port_ftp = 21
    auth_user_ftp = "microcrawler"
    auth_pass_ftp = "rapidev"
    node_ip_wod = "10.100.104.200"
    node_port_wod = 5017
    BASE_URL = "http://10.100.102.114/osint_system/media_files/Object_Detection/"
    calludf = callback({"media_c":["http://10.100.104.114/osint_system/media_files/plane.jpeg","https://media.istockphoto.com/id/162522888/photo/three-pistols.jpg?s=612x612&w=0&k=20&c=Wu4KR_qReBxh4nB3g_QDsH8XoRKGrI9zobkGjPflEkI="]})
    print(calludf)