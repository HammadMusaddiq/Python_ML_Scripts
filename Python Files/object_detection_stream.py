...
def callingWOD(img_arr):
    img_arr = cv2.resize(img_arr, (640, 640), cv2.INTER_AREA)

    api_response = requests.post(f"http://{node_ip_wod}:{node_port_wod}/insert", files = {'image_path': image_toBytes(img_arr)}) # Form data
        ...
        for image_url in list_of_urls:  # for all platforms including reddit
            if image_url != [] and image_url != None and image_url != 'default':
                try:
                    ...
                    wod_data, is_weapon = callingWOD(img_arr)
